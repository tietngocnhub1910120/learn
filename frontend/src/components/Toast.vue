<template>
  <Transition name="bounce">
    <div v-if="isShowToast" class="fixed right-20 top-20">
      <div
        v-if="showSuccess"
        id="toast-success"
...
export default {
  computed: {
    isShowToast() {
      return this.$store.state.toast.isShowToast;
    },
    showMessage() {
      return this.$store.state.toast.message;
    },
    showSuccess() {
      return this.$store.state.toast.success;
    },
  },
  methods: {
    hideToast() {
      this.$store.dispatch("unActiveToast");
    },
  },
};
</script>
